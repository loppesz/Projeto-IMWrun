...
const MOCK_GROUPS = [
  { id: 'g1', name: 'Grupo A', current: 4, max: 6 },
  { id: 'g2', name: 'Grupo B', current: 3, max: 6 },
  { id: 'g3', name: 'Grupo C', current: 0, max: 6 },
];

export default function RaceRegistrationPage() {
  const race = MOCK_RACE;

  const [form, setForm] = useState({ name: '', phone: '', age: '', groupId: '', accepted: false });
  ...
  function validate() {
    ...
    if (!form.groupId)
      e.groupId = 'Escolha um grupo para correr.';
    if (!form.accepted)
            ...
            <form onSubmit={handleSubmit} noValidate className="rounded-2xl bg-white p-6 shadow-md">
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {MOCK_GROUPS.map(g => {
                    const full = g.current >= g.max;
                    return (
                      <button
                        key={g.id}
                        type="button"
                        disabled={full}
                        onClick={() => setForm(f => ({ ...f, groupId: g.id }))}
                      ...
                      >
                        <p>{g.name}</p>
                        <p className="text-xs font-normal mt-0.5">{g.current}/{g.max}</p>
                        {full && <p className="text-xs text-gray-400">Cheio</p>}
                      </button>
                    );
                  })}
                </div>
